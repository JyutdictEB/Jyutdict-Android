...
public final class MarkdownUtil {
    ...
    private static String style(boolean dark) {
        ...
        return "<style>"
                ...
                + "body{width:auto;max-width:100%;margin:0;padding:12px 14px 28px;"
                + "font-family:system-ui,sans-serif;font-size:15px;line-height:1.55;overflow-wrap:anywhere;"
                + "color:" + foreground + ";background:" + background + "}"
                + "article{width:100%;max-width:760px;margin:0 auto}"
                + "h1,h2,h3,h4{font-family:serif;line-height:1.28;margin:1.1em 0 .45em}"
                ...
                + "p{margin:.35em 0 .7em;text-indent:2em}li p,blockquote p,td p,th p{"
                + "text-indent:0;margin:.25em 0}"
                + "strong{color:#D32913}em{color:" + muted + "}"
                + "a{color:" + link + ";text-decoration:underline}"
                + "blockquote{margin:.7em 0;border-left:3px solid #9B2F24;padding:.45em .8em;"
                + "border-radius:0 12px 12px 0;color:" + muted + ";background:" + surface + "}"
                + "img{display:block;max-width:100%;height:auto;margin:.7em auto}"
